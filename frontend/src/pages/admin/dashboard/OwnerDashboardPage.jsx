...
const OwnerDashboardPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch both reports and today's schedule concurrently for speed
        const [reportsRes, bookingsRes] = await Promise.all([
          axiosInstance.get('/api/reservations/reports/'),
          axiosInstance.get('/api/reservations/manage/')
        ]);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* TODAY'S ITINERARY (READ ONLY TABLE) */}
      <h2 className="text-sm font-bold text-gray-900 uppercase tracking-widest mt-8 border-b border-gray-200 pb-2">Today's Itinerary</h2>
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        {todayBookings.length === 0 ? (
            <div className="p-10 text-center text-gray-400 text-sm">No active bookings scheduled for today.</div>
        ) : (
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200 text-[10px] uppercase tracking-widest text-gray-500 font-bold">
                        <tr>
                            <th className="px-6 py-4">Time & Session</th>
                            ...
                            <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 align-middle">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <span className="font-bold text-gray-900 block">{b.time}</span>
                                            <span className="text-[9px] text-gold-600 uppercase tracking-widest font-bold">{b.session}</span>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4 align-middle">
                                    ...
                                    {b.customer_no_show_count > 0 && (
                                        <span className="ml-2 text-[9px] text-rose-600 bg-rose-50 border border-rose-200 px-1.5 py-0.5 rounded uppercase tracking-widest font-bold">
                                            NS: {b.customer_no_show_count}
                                        </span>
                                    )}
                                </td>
                                <td className="px-6 py-4 align-middle text-gray-600 flex items-center gap-2 mt-1">
                                    <MapPin size={14} className="text-gray-400"/> {b.room_name || 'Main Dining Hall'}
                                </td>
                                <td className="px-6 py-4 align-middle text-gray-600">
                                    <span className="flex items-center gap-1.5"><Users size={14} className="text-gray-400"/> {b.pax}</span>
                                </td>
                                <td className="px-6 py-4 align-middle text-right">
                                    <span className={`px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-widest border ${getStatusBadge(b.status)}`}>
                                        {b.status.replace('_', '-')}
                                    </span>
                                </td>
                            </tr>
